...
class ThomasFermiSolver:
    ...
    def optimise(self):
        ...
        # -------------------------------------------------------------
        # Store initial energy before any optimisation steps for reference
        self.energy_history: list[float] = [self.energy(self.nu0.copy())]

        def _bh_callback(x, f, accept):  # noqa: D401 – simple callback
            """Basinhopping callback to log accepted energy values."""
            if accept:
                # Record the energy of the accepted configuration
    ...
    # ------------------------------------------------------------------
    def _plot_energy_decrease(self, out_dir: Path) -> None:  # noqa: D401 – internal helper
        """Save energy and ΔE per iteration plot to *out_dir*.

        Skips the 0-th (initial) energy; ΔE bars start at iteration 2.
        Output filename: ``energy_decrease.png``.
        """
        ...
        iterations = _np.arange(1, energy_full.size)  # 1, 2, ...
        ...
        # ΔE from iteration ≥2
        ax2.bar(iterations[1:], delta_E[1:], color=color_delta, alpha=0.4, label="ΔEnergy (iter≥2)")
