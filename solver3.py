...
class ThomasFermiSolver(_BaseSolver):
    ...
    *non-power-of-two* step if necessary.  After each refinement the *ν*
    ...
    def optimise(self):  # type: ignore[override]
        ...
        # For small grids, fall back to parent implementation ----------------
        if target_N <= 32:
            return super().optimise()

        # Build resolution ladder: 32, 64, ... , target_N
        resolutions: List[int] = []
        n = 32
        while n < target_N:
            resolutions.append(n)
            if n * 2 > target_N:
                # Next doubling would overshoot – jump directly to target
                resolutions.append(target_N)
                break
            n *= 2
        else:
            # target_N is an exact power-of-two ≥32 → we still need to add it
        ...
        for res in resolutions:
            ...
            def _bh_callback(x: np.ndarray, f: float, accept: bool):  # noqa: D401
                if accept:
                    ...
                    # Stop early only for intermediate resolutions
                    if res < target_N:
                        raise _EarlyStop
