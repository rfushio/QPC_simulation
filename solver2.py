...
class SimulationConfig:
    ...
    solver_type: str = "solver2"  # Identifier for which solver implementation is used
...
class ThomasFermiSolver:
    ...
    def _prepare_exc_table(self):
        ...
        self.exc_interp = interp1d(n_exc, Exc_vals, kind="linear", bounds_error=False, fill_value="extrapolate") #type: ignore
    ...
    def gaussian_convolve(self, arr: np.ndarray) -> np.ndarray:
        """Magnetic-length Gaussian smoothing via FFT convolution."""
        # 1) arr → Fourier space with periodic boundary conditions
        arr_q = fft2(arr)
        # 2) Multiply by Gaussian kernel in Fourier space
        conv_q = arr_q * self.G_q
        # 3) Inverse transform and return real part
        conv = ifft2(conv_q)
